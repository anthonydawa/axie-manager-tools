...
import requests
from my_helper import update_VAXH
...
class Claim(AxieGraphQL):
    def __init__(self, acc_name, **kwargs):
        ...
        with open("slp_abi.json", encoding='utf-8') as f:
            slp_abi = json.load(f)
    ...
    async def execute(self):
        ...
        if success:
            blnce = check_balance(self.account)
            rf = f"{self.account.replace('0x', 'ronin:')}"
            logging.info(f"Important: SLP Claimed! New balance for account {self.acc_name} "
                         f"({rf}) is: {blnce}")
            update_VAXH(rf,blnce)
