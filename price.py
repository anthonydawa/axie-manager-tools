...
axs_price = round(float(client.get_avg_price(symbol='AXSUSDT')['price']),3)
slp_price = round(float(client.get_avg_price(symbol='SLPUSDT')['price']),4)
...
print(float(sys.argv[1])*axs_price*51, 'PHP','worth of AXS')
print(float(sys.argv[2])*slp_price*51, 'PHP','worth of SLP')
